document.addEventListener('DOMContentLoaded', () => {
  ...
  const MACHINE_NUM     = 2;  // ← identitas mesin ini
  ...
  async function postPopupDowntimeToDB(type, alasan, durasi_ms, isShared) {
    try {
      ...
      if (type === 'minorbreakdown') {
        // FIX: mesin 2 set field m2 (bukan m1)
        body.minor_durasi_m2_ms = durasi_ms;
        body.minor_alasan_m2    = alasan;
      } else if (isShared) {
        // FIX: shared downtime set m1 DAN m2 (bukan m2 dua kali)
        ...
        // non-shared downtime dari mesin 2 — set field m2
        body.downtime_durasi_m2_ms = durasi_ms;
        body.downtime_alasan_m2    = alasan;
      }
  ...
  // FIX: pakai ws:// bukan mqtt:// — browser hanya support WebSocket
  ...
  console.log(`✅ controle2.js loaded — Mesin ${MACHINE_NUM}`);
});