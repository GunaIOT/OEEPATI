document.addEventListener('DOMContentLoaded', () => {
  ...
  const STORAGE_KEY = 'oee_mesin2_state';
  ...
  function loadState() {
    ...
    } catch(e) { console.warn('[Mesin2] Gagal load state:', e); }
  }

  function clearState() { try { localStorage.removeItem(STORAGE_KEY); } catch(e) {} }

  ;(function cleanLegacyDbKey() {
    ['oee_mesin2_session_id'].forEach(k => { try { localStorage.removeItem(k); } catch(e) {} });
  })();
  ...
  function upgradePopupToBreakdown() {
    ...
    icon.innerText='🔴'; tag.innerText='Breakdown — Mesin 2';
  ...
  function onOneDetected() {
    ...
    if (!inDowntime) {
      ...
      try { mqttClient.publish('oee/machine2/minor', JSON.stringify({minor_total:minorBreakdownAcc}), {retain:false}); } catch(e) {}
    } else {
      if (!popupSubmitted && minorBreakdownWatch > 0) {
        ...
        try { mqttClient.publish('oee/machine2/downtime', JSON.stringify({alasan:'auto-commit',durasi_ms:committedDowntime,downtime_total:downtimeAcc,minor_total:minorBreakdownAcc}),{retain:false}); } catch(e) {}
      }
    }
  ...
  function openPopup(elapsed) {
    ...
    icon.innerText='⚠️'; tag.innerText='Minor Breakdown — Mesin 2';
  ...
  async function postPopupDowntimeToDB(type, alasan, durasi_ms, isShared) {
    try {
      ...
      if (type === 'minorbreakdown') {
        body.minor_durasi_m2_ms = durasi_ms;
        body.minor_alasan_m2    = alasan;
      } else if (isShared) {
  ...
  async function submitDowntime(requireReason) {
    ...
    if (logType === 'downtime') {
      if (isShared) {
        ...
        try { mqttClient.publish('oee/machine2/downtime', JSON.stringify({alasan:cleanLabel,durasi_ms:totalDur,downtime_total:downtimeAcc,minor_total:minorBreakdownAcc})); } catch(e) {}
      }
    } else {
      try { mqttClient.publish('oee/machine2/minor', JSON.stringify({minor_total:minorBreakdownAcc}),{retain:false}); } catch(e) {}
    }
  ...
  mqttClient.on('connect', () => {
    ...
    ['oee/machine2/count','oee/machine2/setup','oee/machine2/lost','oee/machine2/runtime',
     'oee/machine2/target','oee/machine2/reset','oee/machine2/relay-status',
     'oee/machine2/setup-info','oee/shared-downtime'].forEach(t=>mqttClient.subscribe(t));
  });
  ...
  mqttClient.on('message', (topic, message) => {
    ...
    if (topic === 'oee/machine2/relay-status') { if(payload==='ON') handleSensorOn(); if(payload==='OFF') handleSensorOff(); return; }
    ...
    if (topic==='oee/machine2/target')  { const val=parseInt(payload)||0; totalTarget=val; if(targetInput) targetInput.value=val>0?val:''; }
    if (topic==='oee/machine2/reset')   { doReset(); console.log('🔄 RESET received (Mesin 2)'); }
    ...
    if (topic==='oee/shared-downtime') {
      try {
        ...
        if(ev.dari_mesin!==2){
          ...
          if(durasi>downtimeAcc){ downtimeAcc=durasi; addLog('downtime',ev.alasan+' ⚡ \(M1: '+fmtMM(durasi)+')',null,durasi); updateDisplay(); }
        }
      } catch(e) {}
    }
    updateDisplay();
  ...
  document.getElementById('btnSensorOn')?.addEventListener('click',  () => { mqttClient.publish('oee/machine2/relay','ON');  handleSensorOn();  });
  document.getElementById('btnSensorOff')?.addEventListener('click', () => { mqttClient.publish('oee/machine2/relay','OFF'); handleSensorOff(); });

  document.getElementById('btnRestartESP')?.addEventListener('click', async () => {
    if (!confirm('⚠️ Restart ESP32 Mesin 2?\n\n→ Koneksi MQTT putus sementara\n→ Counter di ESP32 reset\n→ Setelah restart: isi shift & produk lalu tekan Simpan Setup\n→ Sistem akan reconnect otomatis')) return;
    showToast('🔄 Mengirim sinyal restart...', '#fb923c');
    mqttClient.publish('oee/machine2/restarted', '1', {retain:true});
    mqttClient.publish('oee/machine2/restart', 'RESTART', {retain:false});
    showToast('🔄 Perintah RESTART ESP32 dikirim...', '#f87171');
    setTimeout(() => window.location.reload(), 4000);
  });

  document.getElementById('btnReset')?.addEventListener('click', () => {
    if (!confirm('⚠️ Reset data produksi Mesin 2?\n\nTarget produksi akan tetap tidak berubah.')) return;
    doReset();
  ...
  document.getElementById('btnSaveSetup')?.addEventListener('click', () => {
    ...
    mqttClient.publish('oee/machine2/setup-info', JSON.stringify({shift:parseInt(shift),product:product||'-',date}), {retain:true});
  ...
  console.log('✅ controle2.js loaded — DB popup downtime aktif');
});