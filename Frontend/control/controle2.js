// controle2.js — Control Panel Mesin Packing #2
// ─────────────────────────────────────────────
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadState() {
    try {
      ...
      console.log('[Mesin2] State loaded — sensorEnabled=' + sensorEnabled);
  ...
  ;(function cleanLegacyDbKey() {
    const legacy = ['oee_mesin2_session_id'];
  ...
  function handleSensorOff() {
    ...
    console.log('[Mesin2] Sensor OFF — semua tracking di-freeze');
  }
  ...
  function freezeAllTracking() {
    ...
    if (popupOpen) {
      closePopup();
      console.log('[Mesin2] Popup ditutup karena sensor OFF');
    }
    ...
    updateDisplay();
    console.log(`[Mesin2] ⏸ Freeze → minor=${(minorBreakdownAcc/1000).toFixed(1)}s  downtime=${(downtimeAcc/1000).toFixed(1)}s`);
  }
  ...
  function upgradePopupToBreakdown() {
    ...
    disableSubmitBtn();
    console.log('[Mesin2] Popup upgraded to Breakdown at', fmtMM(getWatchElapsed()));
  }
  ...
  function openPopup(elapsed) {
    ...
    icon.innerText  = '⚠️';
    tag.innerText   = 'Minor Breakdown — Mesin 2';
  ...
  function submitDowntime(requireReason) {
    ...
    if (logType === 'downtime') {
      if (isShared) {
        try { mqttClient.publish('oee/shared-downtime', JSON.stringify({ alasan: cleanLabel, durasi_ms: totalDur, dari_mesin: 2 }), { retain: false }); } catch(e) {}
      } else {
  ...
  mqttClient.on('connect', () => {
    ...
    mqttClient.subscribe('oee/machine2/setup-info');
      ...
      try {
        ...
        if (ev.dari_mesin !== 2) {
          ...
          if (durasi > downtimeAcc) {
            downtimeAcc = durasi;
            addLog('downtime', ev.alasan + ' ⚡ (M1: ' + fmtMM(durasi) + ')', '', durasi);
            console.log(`[M2] Shared downtime SET ke ${(durasi/1000).toFixed(0)}s`);
            updateDisplay();
          }
        }
  ...
  document.getElementById('btnSensorOn')?.addEventListener('click', () => {
    mqttClient.publish('oee/machine2/relay', 'ON');
    handleSensorOn();
    console.log('[CONTROL] Relay ON sent → Sensor ON (Mesin 2)');
  });

  document.getElementById('btnSensorOff')?.addEventListener('click', () => {
    mqttClient.publish('oee/machine2/relay', 'OFF');
    handleSensorOff();
    console.log('[CONTROL] Relay OFF sent → Sensor OFF (Mesin 2)');
  });
  ...
  document.getElementById('btnRestartESP')?.addEventListener('click', async () => {
    ...
    console.log('[Mesin2] Signal restarted dikirim ke dashboard (retain:true)');
  ...
  document.getElementById('btnSaveSetup')?.addEventListener('click', () => {
    ...
    mqttClient.publish('oee/machine2/setup-info', JSON.stringify({
  ...
  console.log('✅ controle2.js loaded (FIXED v2)');
