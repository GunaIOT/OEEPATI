...
document.addEventListener('DOMContentLoaded', () => {
    ...
    pm.innerHTML = `
      <div class="dt-box">
        <div class="dt-head dt-head-minor" id="popup-dt-head">
          ...
          <div>
            <div class="dt-head-tag" id="popup-dt-tag">Downtime — Mesin 1</div>
  ...
  function openPopup(elapsed) {
    ...
    if (isBreakdown) {
      ...
      icon.innerText = '⚠️';
      tag.innerText  = 'Downtime Minor — Mesin 1';
  ...
  // TOMBOL
  // ════════════════════════════════════════════════════════
  document.getElementById('btnStart')?.addEventListener('click', () => { mqttClient.publish('oee/machine1/status','ON'); });
  document.getElementById('btnStop')?.addEventListener('click',  () => { mqttClient.publish('oee/machine1/status','OFF'); });
  document.getElementById('btnReset')?.addEventListener('click', () => {
    if (!confirm('⚠️ Reset semua data produksi Mesin 1?')) return;
    doReset();
  ...
  document.getElementById('btnDownload')?.addEventListener('click', () => {
    ...
    XLSX.utils.book_append_sheet(wb,ws,'Laporan Mesin 1');
    XLSX.writeFile(wb,`Laporan_Mesin1_${date}_Shift${shift}.xlsx`);
  });
  ...
  console.log('✅ controle.js loaded');
