...
async function insertSession(payload) {
  const {
    ...
    finish_goods = 0, total_reject = 0,
    setup_time_ms = 0, minor_breakdown_ms = 0, downtime_ms = 0,
  } = payload;
  ...
  console.log(`[DB] INSERT session id=${result.insertId} — OEE=${oee.oee}%`);
...
async function updateSession(sessionId, payload) {
  ...
  console.log(`[DB] UPDATE session id=${sessionId} — OEE=${oee.oee}%`);
  return oee;
}
...
module.exports = { insertSession, updateSession, getSessions, calcOEE };