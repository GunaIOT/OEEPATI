...
const { insertSession, updateSession, getSessions, getActiveSession, getSessionById } = require('../services/Session.service');

// ── GET /api/session/active — cek session aktif (untuk multi-device & sync)
router.get('/active', async (req, res) => {
  try {
    const { tgl, shift } = req.query;
    if (!tgl || !shift) return res.status(400).json({ ok: false, error: 'tgl dan shift wajib diisi' });
...
// ── GET /api/session/:id — ambil detail session (untuk restore state di laptop baru)
